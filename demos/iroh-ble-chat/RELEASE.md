...
- `workflow_dispatch` → pick a subset via the `targets` input. The TestFlight job uploads to
  App Store Connect; Android uploads a workflow artifact. Manual runs do not create a GitHub Release.

iOS TestFlight releases are handled by Xcode Cloud in App Store Connect, not by this GitHub
Actions workflow.
...
- `bundle.macOS.bundleVersion` — same monotonic value (TestFlight needs strictly-increasing builds).
...
| `targets`           | Runs                                                              |
| ------------------- | ----------------------------------------------------------------- |
| `all`               | macOS TestFlight + Linux AppImage + Android                      |
| `testflight-all`    | macOS TestFlight upload                                           |
| `macos-testflight`  | macOS MAS only                                                    |
| `linux-appimage`    | Linux AppImage only                                               |
| `android-release`   | Android only                                                      |

Manual runs reuse the `version` in `tauri.conf.json`. The macOS build number is overridden
in CI from the checked-in `bundleVersion` plus the GitHub run number and attempt, so reruns
do not require a version-bump commit. Linux AppImage and Android artifacts use the checked-in
...
iOS TestFlight distribution is intentionally owned by Xcode Cloud. The entry point is
[src-tauri/gen/apple/ci_scripts/ci_post_clone.sh](src-tauri/gen/apple/ci_scripts/ci_post_clone.sh),
and [gen/apple/project.yml](src-tauri/gen/apple/project.yml) keeps the `CI_XCODE_CLOUD` branch
that skips the Tauri prebuild script after Xcode Cloud has built `libapp.a`.
